...
public class PersistenciaAlohandes {
     ...
     */
    private static final Logger log = Logger.getLogger(PersistenciaAlohandes.class.getName());
     ...
     */
    public Operador adicionarOperador(String numeroId, String logIn, String tipoId, String relacionU, int numeroRNT,
                                      String vencimientoRNT, String registroSuperTurismo,
                                      String vencimientoRegistroSuperTurismo, String categoria, String direccion,
                                      String horaApertura, String horaCierre, int tiempoMinimo,
                                      double gananciaAnioActual, double gananciAnioCorrido, ArrayList<?> habitaciones,
                                      ArrayList<?> apartamentos) {
        PersistenceManager pm = pmf.getPersistenceManager();
        Transaction tx = pm.currentTransaction();
        try {
            tx.begin();
            adicionarUsuario(logIn, tipoId, Integer.parseInt(numeroId), relacionU);
            long tuplasInsertadas = sqlOperador.adicionarOperador(pm, numeroId, numeroRNT, vencimientoRNT,
                                                                  registroSuperTurismo, vencimientoRegistroSuperTurismo,
                                                                  categoria, direccion, horaApertura, horaCierre,
                                                                  tiempoMinimo, gananciaAnioActual, gananciAnioCorrido,
                                                                  habitaciones, apartamentos);
            tx.commit();

            log.trace("Inserción operador: " + numeroId + ": " + tuplasInsertadas + " tuplas insertadas");
            return new Operador(logIn, tipoId, numeroId, relacionU, numeroRNT, vencimientoRNT, registroSuperTurismo,
                                vencimientoRegistroSuperTurismo, categoria, direccion, horaApertura, horaCierre,
                                tiempoMinimo, gananciaAnioActual, gananciAnioCorrido, habitaciones, apartamentos);
        } catch (Exception e) {
            //        	e.printStackTrace();
            log.error("Exception : " + e.getMessage() + "\n" + darDetalleException(e));
            return null;
     ...
     */
    public Cliente adicionarCliente(String numeroId, String logIn, String tipoId, String relacionU, String medioPago,
                                    int reservas) {
        PersistenceManager pm = pmf.getPersistenceManager();
        Transaction tx = pm.currentTransaction();
        try {
     ...
     * Adiciona entradas al log de la aplicación
     */
    public Propiedad adicionarPropiedad(long idProp, int capacidad, double precio, double tam, int diasR, int piso,
                                        String fechaCrea) {
        PersistenceManager pm = pmf.getPersistenceManager();
        Transaction tx = pm.currentTransaction();
        try {
            tx.begin();
            long tuplasInsertadas = sqlPropiedad.adicionarPropiedad(pm, idProp, capacidad, precio, tam, diasR,
                                                                    fechaCrea, piso);
            tx.commit();
    ...
    public Propiedad darPropiedadesPorId(long id) {
        return sqlPropiedad.darPropiedadPorId(pmf.getPersistenceManager(), id);
    }
    ...
    public Apartamento adicionarApartamento(long pID, int pCapacidad, double pTamanio, double pPrecio, String pFecha,
                                            int pDiasR, int pPiso, boolean pAmueblado, int pHabitaciones,
                                            String pDMenaje, int pVenceSeguro, String pDSeguro, long pOperador) {
        PersistenceManager pm = pmf.getPersistenceManager();
        Transaction tx = pm.currentTransaction();
        try {
            adicionarPropiedad(pID, pCapacidad, pPrecio, pTamanio, pDiasR, pPiso, pFecha);
            tx.begin();
            long tuplasInsertadas = sqlApartamento.adicionarApartamento(pm, pID, pAmueblado, pDMenaje, pDSeguro,
                                                                        pVenceSeguro, pOperador);
            tx.commit();

            log.trace("Inserción de apartamento: " + pID + ": " + tuplasInsertadas + " tuplas insertadas");


            return new Apartamento(pID, pCapacidad, pTamanio, pPrecio, pFecha, pDiasR, pPiso, pAmueblado, pHabitaciones,
                                   pDMenaje, pVenceSeguro, pDSeguro, pOperador);
        } catch (Exception e) {
            //        	e.printStackTrace();
            log.error("Exception : " + e.getMessage() + "\n" + darDetalleException(e));
            return null;
        ...
        if (activas.isEmpty())
            ...
            if (aptD.size() >= activas.size()) {
                ...
                for (int i = aptD.size() - 1; i <= activas.size(); i++) {
                    ...
                    sqlReservaApartamento.eliminarReservaApartamento(pmf.getPersistenceManager(),
                                                                     activas.get(i).getId());
                }
            }
     ...
     * Adiciona entradas al log de la aplicación
     */
    public Habitacion adicionarHabitacion(long pId, int pCapacidad, double pTamanio, double pPrecio, String pFecha,
                                          int pDiasR, int pPiso, boolean pIndiv, String pEsquema, int pTipo,
                                          long pOperador) {
        PersistenceManager pm = pmf.getPersistenceManager();
        Transaction tx = pm.currentTransaction();
        try {
            ...
            return new Habitacion(pId, pCapacidad, pTamanio, pPrecio, pFecha, pDiasR, pPiso, pIndiv, pEsquema, pTipo,
                                  pOperador);
        } catch (Exception e) {
            //        	e.printStackTrace();
            log.error("Exception : " + e.getMessage() + "\n" + darDetalleException(e));
    ...
    public ReservaColectiva adicionarReservaColectiva(int pCantidad, String pTipo, String pInicio, int pDuracion) {
        ...
        try {
            ...
            long tuplasInsertadas = sqlReservaColectiva.adicionarReservaColectiva(pm, id, pInicio, pDuracion, pCantidad,
                                                                                  pTipo);
            tx.commit();
     ...
     */
    public List<Reserva> adicionarReservaColectivaApartamento(long idColectiva, int pCantidad, String pInicio,
                                                              int pDuracion) {
    ...
    public List<Reserva> adicionarReservaColectivaHabitacion(long idColectiva, int pCantidad, String pInicio,
                                                             int pDuracion) {
    ...
    public List<Long> eliminarReservaColectivaApartamento(long idColectiva) {
        ...
        List<Reserva> reservasC = darReservaPorIdColectiva(idColectiva);
        List<Long> tuplasEliminadas = new ArrayList<>();
        if (reservasC.size() <= 0)
            return null;
        else {
    ...
    public Reserva adicionarReserva(String fechaInicio, String fechaFin, int personas, String finCancelacionOportuna,
        ...
        try {
            tx.begin();
            long id = nextval();
            long tuplasInsertadas = sqlReserva.adicionarReserva(pm, id, fechaInicio, fechaFin, personas,
                                                                finCancelacionOportuna, porcentajeAPagar, montoTotal);
            tx.commit();

            log.trace("Inserción de Reserva: " + id + ": " + tuplasInsertadas + " tuplas insertadas");

            return new Reserva(id, fechaInicio, fechaFin, personas, finCancelacionOportuna, porcentajeAPagar,
                               montoTotal);
        } catch (Exception e) {
            //        	e.printStackTrace();
            log.error("Exception : " + e.getMessage() + "\n" + darDetalleException(e));
    ...
    public List<Object[]> darDineroAnioActual() {
        ...
        Transaction tx = pm.currentTransaction();
        try {
            return sqlOperador.darDineroAnioActual(pm);
        } catch (Exception e) {
            //        	e.printStackTrace();
            log.error("Exception : " + e.getMessage() + "\n" + darDetalleException(e));
            return null;
    ...
    public List<Object[]> darDineroAnioCorrido() {
        ...
        Transaction tx = pm.currentTransaction();
        try {
            return sqlOperador.darDineroAnioCorrido(pm);
        } catch (Exception e) {
            //        	e.printStackTrace();
            log.error("Exception : " + e.getMessage() + "\n" + darDetalleException(e));
            return null;
    ...
    public List<Object[]> darFuncionamientoData() {

        PersistenceManager pm = pmf.getPersistenceManager();
        Transaction tx = pm.currentTransaction();
        try {
            return sqlPropiedad.darFuncionamientoData(pm);
        } catch (Exception e) {
            //        	e.printStackTrace();
            log.error("Exception : " + e.getMessage() + "\n" + darDetalleException(e));
            return null;
    ...
    public List<Propiedad> darOfertaMenorOcupacion() {
        ...
        Transaction tx = pm.currentTransaction();
        try {
            return sqlPropiedad.darOfertaMenorOcupacion(pm);
        } catch (Exception e) {
            //        	e.printStackTrace();
            log.error("Exception : " + e.getMessage() + "\n" + darDetalleException(e));
            return null;
    ...
    public List<Object> darOperadorMenosSolicitado() {
        ...
        Transaction tx = pm.currentTransaction();
        try {
            return sqlPropiedad.darOperadorMenosSolicitado(pm);
        } catch (Exception e) {
            //        	e.printStackTrace();
            log.error("Exception : " + e.getMessage() + "\n" + darDetalleException(e));
            return null;
    ...
    public List<Object> darOperadorMasSolicitado() {
        ...
        Transaction tx = pm.currentTransaction();
        try {
            return sqlPropiedad.darOperadorMasSolicitado(pm);
        } catch (Exception e) {
            //        	e.printStackTrace();
            log.error("Exception : " + e.getMessage() + "\n" + darDetalleException(e));
            return null;
    ...
    public List<Object> darBuenosClientes() {
        ...
        Transaction tx = pm.currentTransaction();
        try {
            return sqlCliente.darBuenosClientes(pm);
        } catch (Exception e) {
            //        	e.printStackTrace();
            log.error("Exception : " + e.getMessage() + "\n" + darDetalleException(e));
            return null;
    ...
    public List<Object[]> consumoSI(String orden, String FI, String FF, String id) {
        PersistenceManager pm = pmf.getPersistenceManager();
        Transaction tx = pm.currentTransaction();

        try {
            return sqlOperador.consumoSI(pm, orden, FI, FF, id);
        } catch (Exception e) {
            //        	e.printStackTrace();
            log.error("Exception : " + e.getMessage() + "\n" + darDetalleException(e));
            return null;
    ...
    public List<Object[]> consumoNO(String orden, String FI, String FF, String id) {
        PersistenceManager pm = pmf.getPersistenceManager();
        Transaction tx = pm.currentTransaction();

        try {

            return sqlOperador.consumoNO(pm, orden, FI, FF, id);
        } catch (Exception e) {
            //        	e.printStackTrace();
            log.error("Exception : " + e.getMessage() + "\n" + darDetalleException(e));
            return null;
