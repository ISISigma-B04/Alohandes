...
class SQLPropiedad {
     ...
     */
    private final PersistenciaAlohandes pa;
     ...
     */
    public long adicionarPropiedad(PersistenceManager pm, long idProp, int capacidad, double precio, double tam,
                                   int diasR, String fechaCrea, int piso) {
        Query q = pm.newQuery(SQL,
                              "INSERT INTO " + pa.darTablaPropiedad() + "(id, capacidad, precio, tamanio, dias_reservados,piso, fecha_creacion) values (?,?,?, ?, ?,?, TO_DATE(?))");
     ...
     */
    public List<Object[]> darFuncionamientoData(PersistenceManager pm) {
        String sql = "WITH ocupacion_semanal AS (";
        sql += " SELECT TO_CHAR(r.fecha_inicio, 'IW') AS semana, ";
        sql += " r.propiedad AS operador, ";
        sql += " COUNT(*) AS cantidad_reservas ";
        sql += " FROM a_reserva r ";
        sql += " GROUP BY TO_CHAR(r.fecha_inicio, 'IW'), r.propiedad ";
        sql += " ), ";

        sql += " extremos_ocupacion AS (";
        sql += " SELECT semana, ";
        sql += " operador, ";
        sql += " MAX(cantidad_reservas) AS max_ocupacion, ";
        sql += " MIN(cantidad_reservas) AS min_ocupacion ";
        sql += " FROM ocupacion_semanal ";
        sql += " GROUP BY semana, operador ";
        sql += " ), ";

        sql += " solicitados AS (";
        sql += " SELECT semana, operador, cantidad_reservas ";
        sql += " FROM ocupacion_semanal ";
        sql += " HAVING (semana, cantidad_reservas) IN (";
        sql += " SELECT semana, MAX(cantidad_reservas) ";
        sql += " FROM ocupacion_semanal ";
        sql += " GROUP BY semana ";
        sql += " UNION ALL ";
        sql += " SELECT semana, MIN(cantidad_reservas) ";
        sql += " FROM ocupacion_semanal ";
        sql += " GROUP BY semana ";
        sql += " ) ";
        sql += " ) ";
        ...
        sql += " SELECT e.semana, ";
        sql += " e.operador AS oferta_max_ocupacion, ";
        sql += " e.max_ocupacion AS max_ocupacion, ";
        sql += " e.operador AS oferta_min_ocupacion, ";
        sql += " e.min_ocupacion AS min_ocupacion, ";
        sql += " s.operador AS operador_max_solicitado, ";
        sql += " s.cantidad_reservas AS max_solicitudes, ";
        sql += " s.operador AS operador_min_solicitado, ";
        sql += " s.cantidad_reservas AS min_solicitudes ";
        sql += " FROM extremos_ocupacion e ";
        sql += " JOIN solicitados s ON e.semana = s.semana";
