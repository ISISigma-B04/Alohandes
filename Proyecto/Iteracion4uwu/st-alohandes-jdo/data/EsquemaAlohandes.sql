...
BEFORE INSERT ON A_RESERVA
FOR EACH ROW
DECLARE
    FECHA_INICIO_OFERTA DATE;
BEGIN
    SELECT fecha_creacion INTO FECHA_INICIO_OFERTA FROM A_OFERTA WHERE ID = :NEW.propiedad;
    IF :NEW.fecha_inicio < FECHA_INICIO_OFERTA THEN
        RAISE_APPLICATION_ERROR(-20000, 'LA FECHA DE INICIO DE LA RESERVA NO PUEDE SER MENOR A LA FECHA DE CREACIÓN DE LA OFERTA');
    END IF;
END;
/
